...
public class Solver {
	...
	public static void main(String[] args){
		...
		for(int j=0;j<total;j++){
			...
			Hand h = new Hand();
			for(int i=0;i<5;i++){
				String cString= h.toString();
				Card card = null;
				do{
					card = new Card(generator.nextInt(14)+1, generator.nextInt(4));
				}
				while(cString.contains(card.toString()));
		...
		*/
		...
		for(int j=0;j<all.size();j+=500){
			System.out.println("all[0] = new hand(\"" + all.get(j).toString() + "\",\"" + all.get(j).ValueToString().substring(0, all.get(j).ValueToString().length()-1)+ "\"," + ((double)j*100)/(double)((all.size())) +"");
		}
		for(int j=all.size()-500+1;j<all.size();j+=20){
			System.out.println("all[0] = new hand(\"" + all.get(j).toString() + "\",\"" + all.get(j).ValueToString().substring(0, all.get(j).ValueToString().length()-1)+ "\"," + ((double)j*100)/(double)((all.size())) +"");
		}
